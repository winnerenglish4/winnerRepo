<template>
    <div>
        ...
        <div class="pageTitle w3-large w3-padding">ผู้ใช้งาน > 
            <router-link to="/user">ผู้ดูแลระบบ</router-link > > แก้ไข
        </div>
        <div class="pageContent">
            <div class="pageContentInside">
                <div class="formdiv">
                    <div class="w3-xlarge w3-padding">ข้อมูลผู้ดูแลระบบ</div>
                    <hr>
...
export default {
    ...
    methods: {
        loadData(){
            //โหลดข้อมูลจาก key id
            db.collection('user').doc(this.$route.params.id)
            .get()
            .then((data)=>{
                this.user =data.data()
            })
        ...
        saveBtn(){
            ...
            db.collection('user').doc(this.$route.params.id).update(this.user)
